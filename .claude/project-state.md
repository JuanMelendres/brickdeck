...
1. Collection UI (closes Phase 2 frontend): `/collection` protected page(s) wrapped in `RequireAuth` — owned sets (list + add-to-collection + status/purchase fields + remove) and loose parts (list + add + quantity/storage + remove), via TanStack Query hooks against `/api/v1/collection/sets` and `/api/v1/collection/parts`. Add nav link once built.
