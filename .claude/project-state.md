...
1. Loose-parts collection UI (closes Phase 2 frontend): loose parts on the `/collection` page (or a tab/section) — list + add (part number + color external id + quantity + storage) + remove, via new hooks against `/api/v1/collection/parts`. Mirror the owned-sets slice. Note backend requires part+color pre-imported via a set inventory (missing → 404) — surface that error.
